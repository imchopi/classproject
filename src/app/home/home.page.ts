...
export class HomePage {
  ...
  onFavClicked(user: User, event: UserInfoFavClicked) {
    ...
    var index = users.findIndex((_usersForTheObservable) => _usersForTheObservable.id == user.id)
    if (index != -1) {
      users[index].fav = event.fav??false
    }
    this._usersForTheObservable.next([...users])
    const options:ToastOptions = {
      message:`${user.name} ${event.fav?'added':'removed'} ${event.fav?'to':'from'} favourites`,
      duration:1000,
      position:'bottom',
      color:'danger',
